...
import { type SchemaLogin, schemaLogin } from "~~/lib/zod/auth";

const toast = useToast();
const { signInWithEmail } = useMyAuthStore();

const loading = ref(false);
const state = reactive<Partial<SchemaLogin>>({
  email: undefined,
  password: undefined,
});
...
async function onSubmit(event: FormSubmitEvent<SchemaLogin>) {
  const credentials = {
